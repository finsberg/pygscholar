...
def test_get_author():
    ...
    with mock.patch("pygscholar.api.scholarly.scholarly.search_author") as m:
        m.return_value = iter([author.dict()])
        a = pygscholar.api.scholarly.get_author(author.name, fill=False)
...
def test_get_author_with_scholar_id():
    ...
    with mock.patch("pygscholar.api.scholarly.scholarly.search_author") as m:
        ...
        a = pygscholar.api.scholarly.get_author(
            author1.name,
...
def test_extract_scholar_publications():
    ...
    with mock.patch("pygscholar.api.scholarly.scholarly") as m:
        ...
        m.fill = lambda x: x
        dep = pygscholar.api.scholarly.extract_scholar_publications(people)
...
def test_find_publication():
    ...
    with mock.patch("pygscholar.api.scholarly.scholarly") as m:
        ...
        m.fill = lambda x: x
        p = pygscholar.api.scholarly.find_publicaton(pub.title)
    assert p == pub
